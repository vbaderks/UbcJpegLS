...
int
write_n_bytes(FILE *output, int value, int n)
{
    int l;
...
#ifdef BIG_ENDIAN
        for (l=n-1;l>=0;l--)  {
        if ( putc((value>>(8*l))&0x000000FF,output) == EOF )
        return EOF;
        }
#else  /* little endian */
    for (l=0;l<n;l++) {
...
int
write_2_bytes(FILE *output, int value)
{
    return write_n_bytes(output, value,2);
}

int
write_marker(FILE *output, int marker)
/* Write a two-byte marker (just the marker identifier) */
{
    write_n_bytes(output, marker, 2);
    return 2;
}

int
write_jpegls_frame(FILE *output, jpeg_ls_header *jp)
{
    int i, marker_len,
    bpp, ct = 0;
         
    ct += write_marker(output, SOF_LS);   /* write JPEG-LS frame marker */

    check_range(jp->comp, "frame components", 1, 255);
    marker_len = 8 + 3*jp->comp;

    ct += write_n_bytes(output, marker_len, 2); /* write marker length */

    for ( bpp=1; (1L<<bpp)<jp->alp; bpp++ );

    ct += write_n_bytes(output, bpp, 1);        /* write bits/sample */
    ...
    ct += write_n_bytes(output, jp->rows, 2);     /* write number of rows */
    ct += write_n_bytes(output, jp->columns, 2);  /* write number of cols */

    ct += write_n_bytes(output, jp->comp, 1);
    ...
    ct += write_n_bytes(output, jp->comp_ids[i], 1); /* component identifier */
    ct += write_n_bytes(output, (sx<<4)|sy, 1);  /* sampling rates */
    ct += write_n_bytes(output, 0, 1);    /* Tq unused */
    }
    return ct;
}
